...
export default function BossTranslatorApp() {
  ...
  const getGlowColor = (level) => {
    // 100分危險等級：使用深紅色漸層，但確保是類似98%的漸層風格，而非全實色，保持通透感但色調為紅
    if (level === 100) return "shadow-[0_0_60px_rgba(220,38,38,0.5)] border-red-500/50 bg-gradient-to-br from-[#2a0505] to-[#050810]";
  ...
  const getTextColor = (level) => {
    if (level === 100) return "text-red-400"; // 提亮顏色，不再使用 animate-pulse
    ...
    <div className="min-h-screen bg-[#050810] font-sans text-slate-300 flex flex-col items-center py-12 px-4 selection:bg-indigo-500/30 overflow-x-hidden relative">
      ...
      <header className="mb-12 text-center max-w-2xl w-full animate-fade-in relative z-10">
        <div className="inline-flex items-center gap-3 mb-6 px-5 py-2 rounded-full bg-slate-900/40 border border-slate-700/50 backdrop-blur-md shadow-lg transition-transform hover:scale-105 duration-500 cursor-default">
          ...
          <span className="text-[11px] tracking-[0.3em] text-indigo-200 uppercase font-light">Soul Reader 18.0</span>
            ...
            <div className={`relative rounded-xl overflow-hidden border transition-all duration-1000 ${getGlowColor(result.puaLevel)}`}>
              ...
              <div className="bg-[#0a0d16]/50 backdrop-blur-sm py-4 px-6 flex justify-between items-center border-b border-white/5 relative z-30">
                <div className="flex items-center gap-3">
                   ...
                   {/* 移除 100% 的 animate-pulse */}
                   <span className={`text-lg font-serif tracking-[0.2em] uppercase drop-shadow-md font-bold ${result.puaLevel === 100 ? 'text-red-400' : 'text-slate-300'}`}>
                     {result.category}
                   </span>
                </div>
              </div>
              ...
              <div className="p-8 md:p-12 relative flex flex-col h-full">
                ...
                <div className="mb-10 text-center relative z-10">
                   {/* 100分危險等級：使用與98%相同的風格(bg-[#2a0505]/40)，而非實色，確保通透感 */}
                   <div className={`relative backdrop-blur-md p-6 rounded-lg border shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] group ${result.puaLevel === 100 ? 'bg-[#2a0505]/40 border-red-500/30' : 'bg-[#1a1f30]/40 border-white/5'}`}>
                        {/* Decorative corners */}
                        ...
                        <Quote className={`w-4 h-4 absolute top-3 left-3 transform -scale-x-100 ${result.puaLevel === 100 ? 'text-red-400/50' : 'text-indigo-400/50'}`} />
                        <p className={`text-lg md:text-xl leading-8 font-serif drop-shadow-md tracking-wide italic ${result.puaLevel === 100 ? 'text-slate-200' : 'text-slate-200'}`}>
                            {result.translation}
                        </p>
                        <Quote className={`w-4 h-4 absolute bottom-3 right-3 ${result.puaLevel === 100 ? 'text-red-400/50' : 'text-indigo-400/50'}`} />
                   </div>
                </div>

                {/* Advice Section */}
                <div className="space-y-6 mb-8">
                   <div className={`p-5 rounded-xl border shadow-inner backdrop-blur-sm relative overflow-hidden group ${result.puaLevel === 100 ? 'bg-[#2a0505]/30 border-red-500/20' : 'bg-[#151a2a]/60 border-white/5'}`}>
                      ...
                      <h4 className={`font-serif mb-2 text-lg tracking-[0.2em] uppercase flex items-center gap-2 font-bold ${result.puaLevel === 100 ? 'text-red-400' : 'text-indigo-300/90'}`}> 
                      ...
                      <p className={`text-sm md:text-base leading-relaxed font-light tracking-wide ${result.puaLevel === 100 ? 'text-slate-300' : 'text-slate-400'}`}>
                        {result.advice}
                      </p>
                   </div>
      ...
      <footer className="mt-auto pt-8 pb-8 text-slate-500 text-[10px] md:text-xs text-center font-light tracking-[0.1em] uppercase opacity-90 relative z-10 hover:opacity-100 transition-opacity duration-500 flex flex-col gap-3">
          <p className="tracking-[0.4em]">Insight & Truth • Ver 18.0</p>
          ...
          <div className="animate-card-flip">
            <div className={`relative rounded-xl overflow-hidden border transition-all duration-1000 ${getGlowColor(result.puaLevel)}`}>
              ...
              <div className="p-8 md:p-12 relative flex flex-col h-full">
                ...
                {/* The Truth - Enhanced Texture */}
                <div className="mb-10 text-center relative z-10">
                   {/* 100分危險等級：修正為更清晰的實色深紅背景 (bg-[#2a0a0a])，移除透明度，確保文字清晰可讀 */}
                ...
                {/* Advice Section */}
                <div className="space-y-6 mb-8">
                   {/* 100分危險等級：修正為更清晰的實色深紅背景 (bg-[#330a0a])，確保文字清晰可讀 */}
